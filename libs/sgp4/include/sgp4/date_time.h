...
#ifndef LIBS_SGP4_INCLUDE_SGP4_DATE_TIME_H_
#define LIBS_SGP4_INCLUDE_SGP4_DATE_TIME_H_
...
namespace datetime {
static int daysInMonth[2][13] = {{0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31,
                                  30, 31},
                                 {0, 31, 29, 31, 30, 31, 30, 31, 31, 30, 31,
                                  30, 31}};
static int cumulDaysInMonth[2][13] = {{0, 0, 31, 59, 90, 120, 151, 181, 212,
                                       243, 273, 304, 334},
...
class DateTime {
 public:
  ...
  DateTime(unsigned int year, double doy) {
    m_encoded = TimeSpan(
        static_cast<int64_t>(AbsoluteDays(year, doy)
                             * time_span::TicksPerDay)).Ticks();
  }
  ...
  static DateTime Now(bool microseconds = false) {
    ...
    if (clock_gettime(CLOCK_REALTIME, &ts) == 0) {
      if (microseconds) {
        dt = DateTime(time_span::UnixEpoch
                      + ts.tv_sec * time_span::TicksPerSecond
                      + ts.tv_nsec / 1000LL * time_span::TicksPerMicrosecond);
      } else {
        dt = DateTime(time_span::UnixEpoch
                      + ts.tv_sec * time_span::TicksPerSecond);
      }
  ...
  static int DaysInMonth(int year, int month) {
    ...
    if (IsLeapYear(year)) {
      daysInMonthPtr = datetime::daysInMonth[1];
    } else {
      daysInMonthPtr = datetime::daysInMonth[0];
    }
  ...
  int DayOfYear(int year, int month, int day) const {
    ...
    if (IsLeapYear(year)) {
      daysThisYear += datetime::cumulDaysInMonth[1][month];
    } else {
      daysThisYear += datetime::cumulDaysInMonth[0][month];
    }
  ...
  /**
   * Get the absolute days
   * @param[in] year The year
   * @param[in] doy The day of the year
   * @returns The absolute days
   */
  ...
  TimeSpan TimeOfDay() const {
    return TimeSpan(Ticks() % time_span::TicksPerDay);
  }
  ...
  int DayOfWeek() const {
     ...
     * 6 Saturday
     */
    return static_cast<int>(((m_encoded / time_span::TicksPerDay) + 1LL) % 7LL);
  }
  ...
  /**
   * Adds years
   * @param[in] years The years
   * @returns new date time
   */
  ...
  DateTime AddMicroseconds(const double microseconds) const {
    int64_t ticks = static_cast<int64_t>(microseconds
                                         * time_span::TicksPerMicrosecond);
  ...
  void FromTicks(int* year, int* month, int* day) const {
    int totalDays = static_cast<int>(m_encoded / time_span::TicksPerDay);
   ...
   */
  int Hour() const {
    return static_cast<int>(m_encoded % time_span::TicksPerDay
                            / time_span::TicksPerHour);
  }
   ...
   */
  int Minute() const {
    return static_cast<int>(m_encoded % time_span::TicksPerHour
                            / time_span::TicksPerMinute);
  }
   ...
   */
  int Second() const {
    return static_cast<int>(m_encoded % time_span::TicksPerMinute
                            / time_span::TicksPerSecond);
  }
  ...
  int Microsecond() const {
    return static_cast<int>(m_encoded % time_span::TicksPerSecond
                            / time_span::TicksPerMicrosecond);
  }
  ...
  double ToGreenwichSiderealTime() const {
    ...
    double gt  = 24110.54841 + t * (8640184.812866 + t *
                                    (0.093104 - t * 6.2E-6));
...
#endif  // LIBS_SGP4_INCLUDE_SGP4_DATE_TIME_H_
