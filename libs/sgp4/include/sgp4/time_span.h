/*
 * Copyright 2013 Daniel Warner <contact@danrw.com>
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#ifndef LIBS_SGP4_INCLUDE_SGP4_TIME_SPAN_H_
#define LIBS_SGP4_INCLUDE_SGP4_TIME_SPAN_H_

#include <cstdint>

#include <cmath>
#include <iomanip>
#include <iostream>
#include <sstream>
#include <string>

namespace osse {
namespace sgp4 {

namespace time_span {
static const int64_t TicksPerDay =  86400000000LL;
static const int64_t TicksPerHour =  3600000000LL;
static const int64_t TicksPerMinute =  60000000LL;
static const int64_t TicksPerSecond =   1000000LL;
static const int64_t TicksPerMillisecond = 1000LL;
static const int64_t TicksPerMicrosecond =    1LL;

static const int64_t UnixEpoch = 62135596800000000LL;

static const int64_t MaxValueTicks = 315537897599999999LL;

// 1582-Oct-15
static const int64_t GregorianStart = 49916304000000000LL;
}  // namespace time_span

/**
 * @brief Represents a time interval.
 *
 * Represents a time interval (duration/elapsed) that is measured as a positive
 * or negative number of days, hours, minutes, seconds, and fractions
 * of a second.
 */
class TimeSpan {
 public:
  /**
   * @brief Constructor
   * @param[in] ticks The ticks
   */
  explicit TimeSpan(int64_t ticks)
      : m_ticks(ticks) {
  }

  /**
   * @brief Constructor
   * @param[in] hours The hours
   * @param[in] minutes The minutes
   * @param[in] seconds The seconds
   */
  TimeSpan(int hours, int minutes, int seconds) {
    CalculateTicks(0, hours, minutes, seconds, 0);
  }

  /**
   * @brief Constructor
   * @param[in] days The days
   * @param[in] hours The hours
   * @param[in] minutes The minutes
   * @param[in] seconds The seconds
   */
  TimeSpan(int days, int hours, int minutes, int seconds) {
    CalculateTicks(days, hours, minutes, seconds, 0);
  }

  /**
   * @brief Constructor
   * @param[in] days The days
   * @param[in] hours The hours
   * @param[in] minutes The minutes
   * @param[in] seconds The seconds
   * @param[in] microseconds The microseconds
   */
  TimeSpan(int days, int hours, int minutes, int seconds, int microseconds) {
    CalculateTicks(days, hours, minutes, seconds, microseconds);
  }

  /**
   * @brief Add
   * @param[in] ts The time span
   * @returns The new time span
   */
  TimeSpan Add(const TimeSpan& ts) const {
    return TimeSpan(m_ticks + ts.m_ticks);
  }

  /**
   * @brief Substract
   * @param[in] ts The time span
   * @returns the new time span
   */
  TimeSpan Subtract(const TimeSpan& ts) const {
    return TimeSpan(m_ticks - ts.m_ticks);
  }

  /**
   * @brief Compares two time spans
   * @param[in] ts The other
   * @returns The comparison
   */
  int Compare(const TimeSpan& ts) const {
    int ret = 0;

    if (m_ticks < ts.m_ticks) {
      ret = -1;
    }
    if (m_ticks > ts.m_ticks) {
      ret = 1;
    }
    return ret;
  }

  /**
   * @brief Compares for equality
   * @param[in] ts The other
   * @returns Whether they are equal
   */
  bool Equals(const TimeSpan& ts) const {
    return m_ticks == ts.m_ticks;
  }

  /**
   * @brief Get the days
   * @returns The days
   */
  int Days() const {
    return static_cast<int>(m_ticks / time_span::TicksPerDay);
  }

  /**
   * @brief Get the hours
   * @returns The hours
   */
  int Hours() const {
    return static_cast<int>(m_ticks % time_span::TicksPerDay
                            / time_span::TicksPerHour);
  }

  /**
   * @brief Get the minutes
   * @returns The minutes
   */
  int Minutes() const {
    return static_cast<int>(m_ticks % time_span::TicksPerHour
                            / time_span::TicksPerMinute);
  }

  /**
   * @brief Get the seconds
   * @returns The seconds
   */
  int Seconds() const {
    return static_cast<int>(m_ticks % time_span::TicksPerMinute
                            / time_span::TicksPerSecond);
  }

  /**
   * @brief Get the milliseconds
   * @returns The milliseconds
   */
  int Milliseconds() const {
    return static_cast<int>(m_ticks % time_span::TicksPerSecond
                            / time_span::TicksPerMillisecond);
  }

  /**
   * @brief Get the microseconds
   * @returns The microseconds
   */
  int Microseconds() const {
    return static_cast<int>(m_ticks % time_span::TicksPerSecond
                            / time_span::TicksPerMicrosecond);
  }

  /**
   * @brief Get the ticks
   * @returns The ticks
   */
  int64_t Ticks() const {
    return m_ticks;
  }

  /**
   * @brief Get the total days
   * @returns The total days
   */
  double TotalDays() const {
    return static_cast<double>(m_ticks) / time_span::TicksPerDay;
  }

  /**
   * @brief Get the total hours
   * @returns The total hours
   */
  double TotalHours() const {
    return static_cast<double>(m_ticks) / time_span::TicksPerHour;
  }

  /**
   * @brief Get the total minutes
   * @returns The total minutes
   */
  double TotalMinutes() const {
    return static_cast<double>(m_ticks) / time_span::TicksPerMinute;
  }

  /**
   * @brief Get the total seconds
   * @returns The total seconds
   */
  double TotalSeconds() const {
    return static_cast<double>(m_ticks) / time_span::TicksPerSecond;
  }

  /**
   * @brief Get the total milliseconds
   * @returns The total milliseconds
   */
  double TotalMilliseconds() const {
    return static_cast<double>(m_ticks) / time_span::TicksPerMillisecond;
  }

  /**
   * @brief Get the total microseconds
   * @returns The total microseconds
   */
  double TotalMicroseconds() const {
    return static_cast<double>(m_ticks) / time_span::TicksPerMicrosecond;
  }

  /**
   * @brief Translate to string
   * @returns The object as a string
   */
  std::string ToString() const {
    std::stringstream ss;

    ss << std::right << std::setfill('0');

    if (m_ticks < 0) {
      ss << '-';
    }

    if (Days() != 0) {
      ss << std::setw(2) << std::abs(Days()) << '.';
    }

    ss << std::setw(2) << std::abs(Hours()) << ':';
    ss << std::setw(2) << std::abs(Minutes()) << ':';
    ss << std::setw(2) << std::abs(Seconds());

    if (Microseconds() != 0) {
      ss << '.' << std::setw(6) << std::abs(Microseconds());
    }

    return ss.str();
  }

 private:
  int64_t m_ticks;

  void CalculateTicks(int days,
                      int hours,
                      int minutes,
                      int seconds,
                      int microseconds) {
    m_ticks = days * time_span::TicksPerDay +
      (hours * 3600LL + minutes * 60LL + seconds) * time_span::TicksPerSecond +
      microseconds * time_span::TicksPerMicrosecond;
  }
};

inline std::ostream& operator<<(std::ostream& strm, const TimeSpan& t) {
  return strm << t.ToString();
}

inline TimeSpan operator+(const TimeSpan& ts1, const TimeSpan& ts2) {
  return ts1.Add(ts2);
}

inline TimeSpan operator-(const TimeSpan& ts1, const TimeSpan& ts2) {
  return ts1.Subtract(ts2);
}

inline bool operator==(const TimeSpan& ts1, const TimeSpan& ts2) {
  return ts1.Equals(ts2);
}

inline bool operator>(const TimeSpan& ts1, const TimeSpan& ts2) {
  return (ts1.Compare(ts2) > 0);
}

inline bool operator>=(const TimeSpan& ts1, const TimeSpan& ts2) {
  return (ts1.Compare(ts2) >= 0);
}

inline bool operator!=(const TimeSpan& ts1, const TimeSpan& ts2) {
  return !ts1.Equals(ts2);
}

inline bool operator<(const TimeSpan& ts1, const TimeSpan& ts2) {
  return (ts1.Compare(ts2) < 0);
}

inline bool operator<=(const TimeSpan& ts1, const TimeSpan& ts2) {
  return (ts1.Compare(ts2) <= 0);
}

}  // namespace sgp4
}  // namespace osse

#endif  // LIBS_SGP4_INCLUDE_SGP4_TIME_SPAN_H_
