...
#include "sgp4/decayed_exception.h"
#include "sgp4/satellite_exception.h"
#include "sgp4/util.h"
...
void SGP4::Initialise() {
  ...
  const double temp3 = 1.25 * kCK4 * pinvsq * pinvsq
    * elements_.RecoveredMeanMotion();
  common_consts_.xmdot = elements_.RecoveredMeanMotion() + 0.5 * temp1
    * betao * common_consts_.x3thm1 + 0.0625 * temp2 * betao *
  ...
  common_consts_.xnodot = xhdot1 + (0.5 * temp2
                                    * (4.0 - 19.0 * theta2) + 2.0 * temp3 *
                                    (3.0 - 7.0 * theta2))
    * common_consts_.cosio;
  ...
  if (use_deep_space_) {
    ...
    nearspace_consts_.c5 = 2.0 * coef1 * elements_.RecoveredSemiMajorAxis()
      * betao2 * (1.0 + 2.75 *
                  (etasq + eeta) + eeta * etasq);
    nearspace_consts_.omgcof = elements_.BStar() * c3
      * cos(elements_.ArgumentPerigee());

    nearspace_consts_.xmcof = 0.0;
    if (elements_.Eccentricity() > 1.0e-4) {
      nearspace_consts_.xmcof = -kTWOTHIRD * coef * elements_.BStar()
        * kAE / eeta;
    }

    nearspace_consts_.delmo = pow(1.0 + common_consts_.eta
                                  * (cos(elements_.MeanAnomoly())), 3.0);
    ...
    if (!use_simple_model_) {
      ...
      nearspace_consts_.d2 = 4.0 * elements_.RecoveredSemiMajorAxis()
        * tsi * c1sq;
      const double temp = nearspace_consts_.d2 * tsi * common_consts_.c1 / 3.0;
      nearspace_consts_.d3 = (17.0 * elements_.RecoveredSemiMajorAxis() + s4)
        * temp;
      nearspace_consts_.d4 = 0.5 * temp * elements_.RecoveredSemiMajorAxis() *
        tsi * (221.0 * elements_.RecoveredSemiMajorAxis() + 31.0 * s4)
        * common_consts_.c1;
      nearspace_consts_.t3cof = nearspace_consts_.d2 + 2.0 * c1sq;
      nearspace_consts_.t4cof = 0.25 * (3.0 * nearspace_consts_.d3
                                        + common_consts_.c1 *
                                        (12.0 * nearspace_consts_.d2
                                         + 10.0 * c1sq));
      nearspace_consts_.t5cof = 0.2 * (3.0 * nearspace_consts_.d4 + 12.0
                                       * common_consts_.c1 *
                                       nearspace_consts_.d3 + 6.0
                                       * nearspace_consts_.d2
                                       * nearspace_consts_.d2 + 15.0 *
                                       c1sq * (2.0 * nearspace_consts_.d2
                                               + c1sq));
    }
  }
}
...
void SGP4::RecomputeConstants(const double xinc,
  ...
  if (fabs((*cosio) + 1.0) > 1.5e-12) {
    (*xlcof) = 0.125 * kA3OVK2 * (*sinio) * (3.0 + 5.0 * (*cosio))
      / (1.0 + (*cosio));
  } else {
    ...
    if (elements_.RecoveredMeanMotion() < 0.0052359877
      ...
      if (elements_.Eccentricity() < 0.7) {
        ...
        g521 = EvaluateCubicPolynomial(elements_.Eccentricity(),
                                       -822.71072, 4568.6173, -8491.4146,
                                       5337.524);
        g532 = EvaluateCubicPolynomial(elements_.Eccentricity(),
                                       -853.666, 4690.25, -8624.77, 5341.4);
      } else {
        g533 = EvaluateCubicPolynomial(elements_.Eccentricity(),
                                       -37995.78, 161616.52, -229838.2,
                                       109377.94);
        g521 = EvaluateCubicPolynomial(elements_.Eccentricity(),
                                       -51752.104, 218913.95, -309468.16,
                                       146349.42);
        g532 = EvaluateCubicPolynomial(elements_.Eccentricity(),
                                       -40023.88, 170470.89, -242699.48,
                                       115605.82);
      }
  ...
  if (ds_constants.shape != DeepSpaceConstants::NONE) {
    ...
    while (running) {
      ...
      if (ds_constants.shape == DeepSpaceConstants::SYNCHRONOUS) {
        ...
        // TODO(dan): check
        const double xomi = elements.ArgumentPerigee() + c_constants.omgdot
          * integ_params->atime;
      ...
      if (fabs(ft) >= STEP) {
        ...
        const double theta = Util::WrapTwoPI(ds_constants.gsto + tsince
                                             * kTHDT);
        if (ds_constants.shape == DeepSpaceConstants::SYNCHRONOUS) {
